...
public class AlbumHelper {
	...
	public AlbumBean getSystemAlbum(List<AlbumBean> beans) {
		if (!beans.isEmpty()) {
			for (AlbumBean bean : beans) {
				if (bean.folderName.equals(PATH_CAMERA)) {
					return bean;
				}
			}
			return beans.get(0);
	...
	public List<AlbumBean> getFolders() {
		...
		Cursor mCursor = contentResolver.query(mImageUri, null, MediaStore.Images.Media.MIME_TYPE + "=? or " + MediaStore.Images.Media.MIME_TYPE
		...
		for (Iterator<Map.Entry<String, List<ImageBean>>> iterator = set.iterator(); iterator.hasNext();) {
			Map.Entry<String, List<ImageBean>> entry = iterator.next();
			String parentName = entry.getKey();
			ImageBean b = entry.getValue().get(0);
			AlbumBean tempAlbumBean = new AlbumBean(parentName, entry.getValue().size() + 1, entry.getValue(), b.path);
